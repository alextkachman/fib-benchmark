class BinaryTrees {
    ...
    private static class TreeNode
    {
        ...
        private static TreeNode bottomUpTree(int item, int depth){
            depth ? new TreeNode(left:bottomUpTree(2*item-1, depth-1), right:bottomUpTree(2*item, depth-1), item:item) : new TreeNode(item:item)
        }
        ...
        static iterateDepth (int maxDepth, int depth) {
            int iterations = 1 << (maxDepth - depth + BinaryTrees.minDepth)
            int check = 0
