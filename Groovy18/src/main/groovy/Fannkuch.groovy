...
class Fannkuch implements Runnable {
	private final int n
	private final int[] flip_max_arr
	private final AtomicInteger remain_task = new AtomicInteger(0)

	static void main(String[] args) {
		int x = (args.length > 0) ? Integer.parseInt(args[0]) : 12
//        println "ATTN: Groovy version uses 11 instead of 12"
		Fannkuch f = new Fannkuch(x)
		long millis = System.currentTimeMillis()
		System.out.format("Pfannkuchen(%d) = %d\n", x, f.fank_game())
		long total = System.currentTimeMillis() - millis
        System.out.println("[Fannkuch-" + System.getProperty("project.name")+ " Benchmark Result: " + total + "]");
	}
